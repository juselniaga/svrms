    ...
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <h2 class="font-semibold text-xl text-gray-800 leading-tight block">
                {{ __('Edit Application: ') . $application->reference_no }}
            </h2>
        </div>
    </x-slot>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6">
                ...
                <div class="mb-8 p-4 bg-gray-50 rounded-lg border border-gray-200">
                    <h3 class="font-medium text-gray-900 mb-2">Developer Information</h3>
                    <div class="grid grid-cols-2 gap-4 text-sm">
                        <div><span class="text-gray-500">Name:</span> <span
                                class="font-semibold">{{ $application->developer->name }}</span></div>
                        <div><span class="text-gray-500">Email:</span> <span>{{ $application->developer->email }}</span>
                        </div>
                        <div><span class="text-gray-500">Phone:</span> <span>{{ $application->developer->tel }}</span>
                        </div>
                        <div><span class="text-gray-500">Location:</span> <span>{{ $application->developer->city }},
                                {{ $application->developer->state }}</span></div>
                    </div>
                </div>

                <form method="POST" action="{{ route('clerk.applications.update', $application->application_id) }}"
                    x-data="{
                    tajuk: '{{ old('application.tajuk', $application->tajuk) }}',
                    lokasi: '{{ old('application.lokasi', $application->lokasi) }}'
                }">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                        ...
                        <div>
                            ...
                            <x-text-input id="application_no_fail" class="block mt-1 w-full" type="text"
                                name="application[no_fail]" :value="old('application.no_fail', $application->no_fail)"
                                required />
                        ...
                        <div>
                            ...
                            <select id="application_officer_id" name="application[officer_id]" required
                                ...
                                @foreach($officers as $officer)
                                    <option value="{{ $officer->user_id }}" {{ old('application.officer_id', $application->officer_id) == $officer->user_id ? 'selected' : '' }}>
