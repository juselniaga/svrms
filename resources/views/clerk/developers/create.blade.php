        ...
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6">
                ...
                <form method="POST" action="{{ route('clerk.developers.store') }}" x-data="{
                    ...
                    <h3 class="text-lg font-medium text-gray-900 mb-4 border-b pb-2">Developer Information</h3>
                    <p class="text-sm text-gray-500 mb-6">Register a new Developer profile to associate with the upcoming application.</p>

                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                        <div>
                            <x-input-label for="name" :value="__('Company/Developer Name *')" />
                            <x-text-input id="name" class="block mt-1 w-full" type="text"
                                name="name" x-model="developerName" required autofocus />
                            <x-input-error :messages="$errors->get('name')" class="mt-2" />
                            <p x-show="!developerName" class="text-sm text-red-500 mt-1" style="display: none;">Name is required.</p>
                        </div>
                        <div>
                            ...
                            <p class="text-xs text-gray-500 mt-1">Email address must be unique.</p>
                        </div>
                        <div>
                        ...
                        <div>
                            <x-input-label for="poskod" :value="__('Postcode *')" />
                    ...
                    <div class="flex items-center justify-end mt-4 pt-4 border-t">
                        ...
                        <x-primary-button>
                            {{ __('Save & Continue to Application') }}
                        </x-primary-button>
                    </div>
                </form>
            </div>
