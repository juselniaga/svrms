        ...
        <div class="max-w-2xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    <form method="POST" action="{{ route('admin.bpks.update', $bpk) }}" class="space-y-6">
                        @csrf
                        @method('PUT')
                        ...
                        <div>
                            ...
                            <select name="bp_id" id="bp_id"
                                ...
                                @foreach($bps as $bp)
                                    <option value="{{ $bp->id }}" {{ old('bp_id', $bpk->bp_id) == $bp->id ? 'selected' : '' }}>
                                        {{ $bp->id }} - {{ $bp->bp_name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            ...
                            <input type="text" name="bpk_short" id="bpk_short"
                                value="{{ old('bpk_short', $bpk->bpk_short) }}"
                                class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500"
                        ...
                        <div>
                            ...
                            <input type="text" name="bpk_name" id="bpk_name"
                                value="{{ old('bpk_name', $bpk->bpk_name) }}"
                                class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500"
